...
### 🔌 دليل الدمج والتشغيل (How to Use):
لتشغيل وتفعيل هذه الأداة في بيئة عمل المساعد الذكي الخاص بك (مثل Claude Desktop أو Gemini Code Assist أو أي بيئة عمل برمجية ذكية أخرى)، قم بنسخ مجلد `doi-validator/` أو ملف المهارة المعبأ `doi-validator.skill` إلى مجلد الإضافات المحلي لديك:

```bash
# مسار مجلد إضافات المساعد الذكي المحلي على جهازك
~/.config/ai-agents/skills/
```

بمجرد تحميل الأداة بنجاح، سيقوم المساعد الذكي تلقائياً بتنفيذ خطوات التحقق والتوثيق المباشرة فور طلبك صياغة قائمة المراجع، التحقق من صحة توثيق علمي، أو البحث عن أوراق بحثية موثقة.
