...
After fetching real metadata, format the reference in two styles: **APA 7th Edition** (for text representation) and **RIS Format** (for reference manager software imports like Zotero, Mendeley, or EndNote).

#### 1. APA 7th Edition Standard
```
LastName, F. M., LastName2, F. M., & LastName3, F. M. (YEAR). Title of article. Journal Name, Volume(Issue), StartPage–EndPage. https://doi.org/XX.XXXXX/XXXXX
```

**APA 7th Rules:**
- **Authors:** List up to 20 authors. Use an ampersand (`&`) before the final author. For 21+ authors, list the first 19, insert an ellipsis (`...`), and add the final author (do not use `&`).
- **Capitalization:** Use sentence case for the article title (capitalize only the first word of the title and subtitle, and any proper nouns).
- **Styling:** Journal name and volume must be *italicized* (or plain in raw text, but preserved in markdown).
- **DOI:** Must be formatted as a complete URL: `https://doi.org/XX.XXXXX/XXXXX`.

#### 2. RIS Format (Master Reference Software Output)
Provide a raw text block in RIS format so the user can easily save it as a `.ris` file:
```text
TY  - JOUR
AU  - LastName, GivenName M.
AU  - LastName2, GivenName2 M.
TI  - Title of article
JO  - Journal Name
PY  - YEAR
VL  - Volume
...
EP  - EndPage
DO  - XX.XXXXX/XXXXX
UR  - https://doi.org/XX.XXXXX/XXXXX
ER  - 
```

**RIS Rules:**
- `TY  - JOUR` defines the reference type as a journal article.
- Use a separate `AU  - ` tag line for each author.
- Ensure the record ends with the exact tag `ER  - ` on its own line to signal the End of Record.
